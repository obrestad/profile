...
if [[ $# -lt 4 ]]; then
  ...
  echo "Usage: $0 <remote-user> <remote-host> <backup-path> <remote-paths>"
  exit 1
fi
...
if [[ -z $lastBackup ]]; then
  ...
  for path in $paths; do
    ...
    rsync -a -v "${remote}:${path}" $backupPath 2> /dev/null >> $output
  done
else
  ...
  for path in $paths; do
    echo "Backing up $path" >> $output
    logger "Backing up $path"
    rsync -a -v "${remote}:${path}" $backupPath --link-dest=$link 2> /dev/null >> $output
  done
fi
