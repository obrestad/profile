...
remotePath="/srv/backup/general/$hostname"
...
if [[ -z $lastBackup ]]; then
  ...
  rsync -a -v /etc $remote 2> /dev/null >> $output
  rsync -a -v /root $remote 2> /dev/null >> $output
  rsync -a -v /home $remote 2> /dev/null >> $output
else
  echo "Performing an incremental backup" >> $output
  link="../$lastBackup/"
  rsync -a -v /etc $remote --link-dest=$link 2> /dev/null >> $output
  rsync -a -v /root $remote --link-dest=$link 2> /dev/null >> $output
  rsync -a -v /home $remote --link-dest=$link 2> /dev/null >> $output
fi
