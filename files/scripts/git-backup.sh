...
remotePath="/srv/backup/git/$hostname"
...
if [[ $1 == "-v" ]]; then
  ...
  output=/tmp/autobackup.git.$date.log
fi
...
if [[ -z $lastBackup ]]; then
  ...
  rsync -a -v /srv/git/ $remote 2> /dev/null >> $output
else
  echo "Performing an incremental backup" >> $output
  link="../$lastBackup/"
  rsync -a -v /srv/git/ $remote --link-dest=$link 2> /dev/null >> $output
fi
